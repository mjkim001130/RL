...
parser.add_argument(
    ...
    type=str,
    default="cuda",
    help="running device of algorithm: cuda or cpu",
)
parser.add_argument(
    "--write",
    type=str2bool,
...
parser.add_argument(
    "--Max_train_steps", type=int, default=int(1e6), help="Max training steps"
)
parser.add_argument(
    "--save_interval",
    type=int,
...
parser.add_argument(
    "--buffersize", type=int, default=int(1e4), help="Size of the replay buffer"
)
parser.add_argument(
    "--target_freq", type=int, default=int(1e3), help="frequency of target net updating"
)

parser.add_argument("--gamma", type=float, default=0.99, help="Discounted Factor")
parser.add_argument("--lr", type=float, default=1e-4, help="Learning rate")
parser.add_argument(
    "--batch_size", type=int, default=32, help="lenth of sliced trajectory"
)
parser.add_argument("--init_e", type=float, default=1.0, help="Initial e-greedy noise")
parser.add_argument(
    "--anneal_frac", type=int, default=3e5, help="annealing fraction of e-greedy noise"
)
parser.add_argument("--final_e", type=float, default=0.02, help="Final e-greedy noise")
parser.add_argument(
    "--noop_reset", type=str2bool, default=False, help="use NoopResetEnv or not"
)
parser.add_argument(
    "--huber_loss",
    type=str2bool,
    default=True,
    help="True: use huber_loss; False:use mse_loss",
)
parser.add_argument(
    "--fc_width", type=int, default=200, help="number of units in Fully Connected layer"
)

parser.add_argument(
    "--EnvIdex", type=int, default=37, help="Index of the Env; 20=Enduro; 37=Pong"
)
parser.add_argument("--seed", type=int, default=5, help="random seed")
parser.add_argument(
    "--Double", type=str2bool, default=False, help="whether to use Double Q-learning"
)
parser.add_argument(
    "--Duel", type=str2bool, default=False, help="whether to use Duel. Q-learning"
)
parser.add_argument(
    "--Noisy", type=str2bool, default=False, help="whether to use NoisyNet"
)
opt = parser.parse_args()
opt.dvc = torch.device(opt.device)
opt.algo_name = (
    ("Double-" if opt.Double else "")
    + ("Duel-" if opt.Duel else "")
    + ("Noisy-" if opt.Noisy else "")
    + "DQN"
)
...
def main():
    ...
    if opt.render:
        ...
        schedualer = LinearSchedule(
            schedule_timesteps=opt.anneal_frac,
            final_p=opt.final_e,
            initial_p=opt.init_e,
        )
        ...
        while total_steps < opt.Max_train_steps:
            ...
            while not done:
                ...
                s_next, r, dw, tr, info = env.step(
                    a
                )  # dw(dead & win): terminated; tr: truncated
                ...
                if buffer.size >= opt.random_steps:
                    ...
                    if total_steps % opt.eval_interval == 0:
                        score = evaluate_policy(
                            eval_env, agent, seed=seed + 1
                        )  # 不与当前训练seed重合，更general
                        if opt.write:
                        ...
                        print(
                            f"{opt.ExperimentName}, Seed:{opt.seed}, Step:{int(total_steps/1000)}k, Score:{score}"
                        )
                        agent.exp_noise = schedualer.value(
                            total_steps
                        )  # e-greedy decay
